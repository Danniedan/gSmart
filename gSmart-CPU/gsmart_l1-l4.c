...
int main(int argc, char** argv)
{
  ...
  long nnz, i, j, it, nzr, nzc;	
	...
	char * filename1="./data/wat100r_l1.txt";
	char * filename2="./data/wat100c_l1.txt";
  ...
  for (i=0; i<nnz; i++)
  {
	...
	//constants 47739, 138735
	int con[num_con] = {54504};
	int con_pre[num_con] = {2};
	int flag;
	...
	int *bind3 = (int *) malloc(num_pre_l3*k*maxl_r*maxl_r * sizeof(int));
	int edge_pre_l3[num_pre_l3]={0};
	...
	for(i=0; i<k; i++)
	{
		...
		if(flag==1)
		{
			//evaluate 1th-level query edges
				...
				for(g=0; g<num_pre_l3; g++)
				{
					...
					if( (Mr[bind2[i*maxl_r+p]+1]-Mr[bind2[i*maxl_r+p]])==1 )
					{
						for(j=Ap_r[Mr[bind2[i*maxl_r+p]]]; j<Ap_r[Mr[bind2[i*maxl_r+p]]+1]; j++)
						{
							...
							{
								bind3[g*k*maxl_r*maxl_r+i*maxl_r*maxl_r+ind_l2[0]*maxl_r+ind_l3[g]] = obs_r[j];
								ind_l3[g]++;
								flag=1;
							}
						}
