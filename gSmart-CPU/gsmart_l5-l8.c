...
int main(int argc, char** argv)
{
  ...
  int M;
	...
	char * filename1="./data/wat100r_l5.txt";
	char * filename2="./data/wat100c_l5.txt";
  ...
  for (i=0; i<nnz; i++)
  {
	...
	//constants 52, 16, 184, 
	int con[num_con] = {274, 169};
	int con_pre[num_con] = {1, 0};
	int flag;
	...
	if( (Mr[con[1]+1]-Mr[con[1]])==1 )
	{
		k=0;
		for(i=Ap_r[Mr[con[1]]]; i<Ap_r[Mr[con[1]]+1]; i++)
		{
			if( pres_r[i]==con_pre[1] )
			{
				bind1_2[obs_r[i]] = 1;
				k++;
			}
	...
	for(i=0; i<Bp1[0]; i++)
	{
		if( (bind1_1[i]!=-1))
		{
			for(j=0; j<maxl_r; j++)
			{
				if( (bind2[i*maxl_r+j]!=-1) )
				{
					bind2_1[bind2[i*maxl_r+j]]=1;	
				}
			}
		}
	}
